...
public class SkillStanceDance extends ActiveSkill {

    public SkillStanceDance(Heroes plugin) {
        super(plugin, "StanceDance");
        setDescription("Swaps from one stance to the other");
        setUsage("/skill StanceDance");
        
        setArgumentRange(0, 1);
        setIdentifiers("skill StanceDance");
        
        setTypes(SkillType.FIRE);
    }
    ...
    public SkillResult use(Hero hero, String[] args) {
        ...
        // APPLY DRAGON
        if(hero.hasEffect("CraneStanceEffect") || (!hero.hasEffect("DragonStanceEffect") && !hero.hasEffect("CraneStanceEffect"))) {
            hero.removeEffect(hero.getEffect("CraneStanceEffect"));
            hero.addEffect(new com.herocraftonline.heroes.characters.effects.Effect(this,"DragonStanceEffect"));
            ...
            plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {
                @Override
                public void run() {
                    fhero.getPlayer().getWorld().playEffect(new Location(fhero.getPlayer().getWorld(),fhero.getPlayer().getLocation().getX(),fhero.getPlayer().getLocation().getY()+(ii-1),fhero.getPlayer().getLocation().getZ()), Effect.POTION_BREAK, 245);
                }
            }, i);
            }
        }
        // APPLY CRANE
        else if(hero.hasEffect("DragonStanceEffect")) {
